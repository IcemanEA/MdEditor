...
final class OpenDocFilesWorker: IOpenDocFilesWorker {
	...
	func getFiles(atPath path: URL?) -> [File] {
		...
		do {
			...
			// swiftlint:disable:one print_using
...
extension FilesManager.Error: LocalizedError {
	var errorDescription: String? {
		switch self {
		case .fileNotExist:
			return L10n.FilesManager.Error.fileNotExist
		case .unKnowAttributes:
			return L10n.FilesManager.Error.unKnowAttributes
		case .canNotCreateFileExist:
			return L10n.FilesManager.Error.canNotCreateFileExist
		}
	}
}
