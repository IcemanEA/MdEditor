...
final class MainRouter: IMainRouter {
	...
	func showNewDocScreen() {
		let alert = UIAlertController.createAlert(withTitle: L10n.Localizable.MainMenu.newDocument, andMessage: "")
		alert.action(withText: L10n.Localizable.Alert.PlaceHolder.newDoc) { [weak self] name in
			do {
				if let url = try self?.filesManager.createFile(with: name, and: "md") {
					self?.showFile(url)
				}
			} catch {
				self?.showAlert(L10n.Localizable.Alert.Title.warning, withMessage: error.localizedDescription)
			}
		}
	...
	private func showAlert(_ title: String, withMessage message: String = "") {
		...
		let action = UIAlertAction(title: L10n.Localizable.Alert.Button.ok, style: .cancel)
