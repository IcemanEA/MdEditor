...
extension UIAlertController {
	...
	func action(withText placeholder: String, completion: @escaping (String) -> Void) {
		let addAlertAction = UIAlertAction(title: L10n.Localizable.Alert.Button.add, style: .default) { _ in
			guard let newValue = self.textFields?.first?.text else { return }
			guard !newValue.isEmpty else { return }
			completion(newValue)
		}
		
		let cancelAction = UIAlertAction(title: L10n.Localizable.Alert.Button.cancel, style: .destructive)
